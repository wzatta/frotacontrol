...
public class RegisterRequestService {
	...
	public RegisterRequestService(RegisterRequestMapper mapper, UsuarioLogadoService userLogado,
	 ...
	 */
	@Transactional
	public Usuario createUser(RegisterRequestDto dto) {
	...
	private Empresa verificarEmpresa(RegisterRequestDto dto) {
	 ...
	 */
	@Transactional
	public Usuario updateUser(Long id, RegisterRequestDto dto) {
	...
	private void validarRolesCriacaoUsuario(List<Role> rolesLogado, List<Role> rolesNovoUsuario) {
		...
		Set<Role> forbiddenRoles = Set.of(Role.ROLE_ADMIN, Role.ROLE_ADMIN_EMPRESA);

		for (Role role : rolesNovoUsuario) {
			if (forbiddenRoles.contains(role)) {
				throw new RuntimeException("ROLE_ADMIN_EMPRESA não pode atribuir: " + role);
			}
		}
	}
