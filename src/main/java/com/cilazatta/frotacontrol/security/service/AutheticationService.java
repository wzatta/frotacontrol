...
import com.cilazatta.frotacontrol.exeptions.ColaboradorNotFoundException;
...
public class AutheticationService {
    ...
    public AutheticationService(AuthenticationManager manager,
                                UsuarioRepository userRepo,
                                JwtService jwtService,
                                FuncionarioRepository colabRepo,
	...
	public AutheticationResponseDto register(RegisterRequestDto requestRegister) {

		Funcionario colab = colabRepo.findById(requestRegister.getFuncionarioId())
		        .orElseThrow(() -> new ColaboradorNotFoundException(requestRegister.getFuncionarioId()));
		
		var usuario = Usuario.builder()
				.name(requestRegister.getName())
				.login(requestRegister.getLogin())
				.funcionario(colab)
				.ativo(requestRegister.getAtivo())
				.senha(this.encoder.encode(requestRegister.getPassword()))
		...
		return AutheticationResponseDto.builder()
				.token(jwtToken)
				.build();
	...
	public AutheticationResponseDto authetication(AutheticateRequestDto requestAuth) {
        ...
        try {
            manager.authenticate(
                new UsernamePasswordAuthenticationToken(login, requestAuth.getPassword())
            );
